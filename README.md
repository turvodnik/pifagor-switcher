...
- Консервативное live-исправление после короткой паузы во время набора.
...
- Глобальные хоткеи:
  ...
  - двойной `Option` — исправить только текущее или последнее завершенное слово;
...
## Live-автоисправление

Live-режим включен по умолчанию и срабатывает только после короткой паузы в наборе. Он использует более высокий порог уверенности, чем исправление на пробеле, и отменяется при Backspace, Enter, Escape, пробеле, пунктуации, движении курсора, клике мышью или смене приложения.

Live-режим не работает в `manualOnly`, `disabled`, Terminal/iTerm, secure input и исключенных приложениях. Технические токены вроде `github`, `wp-admin`, `user_id`, URL, email, путей, camelCase и acronyms защищены от live-исправления.
...
- `PifagorSwitcher-vX.Y.Z.zip`

Для реального распространения zip нужно подписать Developer ID сертификатом и notarize через Apple.

Публичные версии и архивы для тестировщиков публикуются в GitHub Releases: `https://github.com/turvodnik/pifagor-switcher/releases`.
